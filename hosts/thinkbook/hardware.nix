...
{ config, pkgs, lib, ... }:
{
  ...
  hardware.nvidia = {
    ...
    powerManagement.enable      = false;  # Enable if suspend/resume causes issues
    powerManagement.finegrained = false;  # Turns off GPU when idle (experimental)
    open           = true;   # Open-source kernel module (required for Turing+)
